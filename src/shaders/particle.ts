...
import { pbrFunctions } from './pbr.ts';

export const particleVS: string = `#version 300 es
...
export const particleFS: string = `#version 300 es
precision highp float;
in float v_depth;
in float v_hash;
in float v_energy;
...
void main(){
  ...
  for (int i = 0; i < u_lightCount && i < 8; i++) {
    vec3 toLight = u_lightPositions[i] - v_world;
