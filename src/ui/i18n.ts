...
export interface TranslationStrings {
  subtitle: string;
  shapes_flight: string;
  shapes: string;
  modes: string;
  mode_shapes: string;
  mode_free: string;
  mode_fractal: string;
  mode_equalizer: string;
  colors: string;
  color_count: string;
  shuffle_palette: string;
  shape_morphing: string;
  auto_switch: string;
  auto_speed: string;
  custom_transition: string;
  shape_attraction: string;
  force: string;
  active_cursor: string;
  cursor_hint: string;
  enable_cursor: string;
  mode: string;
  radius: string;
  interaction_type: string;
  cursor_attract: string;
  cursor_repel: string;
  cursor_vortex_left: string;
  cursor_vortex_right: string;
  cursor_pulse: string;
  cursor_quasar: string;
  cursor_magnet: string;
  cursor_strength: string;
  influence_radius: string;
  pulse_on_press: string;
  particles: string;
  particle_count_label: string;
  particles_word: string;
  active: string;
  particle_speed: string;
  ...
  mid: string;
  treble: string;
  bass_sensitivity: string;
  mid_sensitivity: string;
  treble_sensitivity: string;
  speed_slow: string;
  speed_normal: string;
  speed_fast: string;
  speed_custom: string;
...
type CursorModeKey = 'attract' | 'repel' | 'vortex-left' | 'vortex-right' | 'pulse' | 'quasar' | 'magnet';

const cursorModeNames: Record<Language, Record<CursorModeKey, string>> = {
  ru: {
    attract: 'Притягивать',
    repel: 'Отталкивать',
...
export interface I18n {
  switchLanguage: (lang: Language) => { SHAPE_NAMES: readonly string[]; currentLang: Language };
  updateCursorModeLabel: (pointerMode: string) => void;
  getCurrentLang: () => Language;
  getShapeNames: () => readonly string[];
}

export function createI18n(): I18n {
  let currentLang: Language = 'ru';
  let SHAPE_NAMES: readonly string[] = SHAPE_NAMES_RU;

  const updateCursorModeLabel = (pointerMode: string): void => {
    const cursorModeLabel: HTMLElement | null = document.getElementById('cursorModeLabel');
    if (cursorModeLabel) {
      const modeKey = pointerMode as CursorModeKey;
      cursorModeLabel.textContent = cursorModeNames[currentLang][modeKey] || cursorModeNames[currentLang]['attract'];
    }
  };

  const switchLanguage = (lang: Language): { SHAPE_NAMES: readonly string[]; currentLang: Language } => {
    currentLang = lang;
    ...
    document.querySelectorAll('[data-i18n-opt]').forEach((el: Element) => {
      const key = el.getAttribute('data-i18n-opt') as TranslationKey | null;
      if (key && t[key]) {
        el.textContent = t[key];
      }
    });

    const langBtn: HTMLElement | null = document.getElementById('langToggle');
    if (langBtn) {
  ...
  return {
    ...
    getCurrentLang: (): Language => currentLang,
    getShapeNames: (): readonly string[] => SHAPE_NAMES
  };
}
